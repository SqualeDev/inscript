<?php
    ...
    foreach ($arrDates as $key => $value) {
        ...
        foreach ($arrDepDuJour as $keyDuJour => $depDuJour) {
            ...
            if (!empty($arrDepartsUser)) {
                ...
                for ($i=0; $i < $intNbValue; $i++) {
                    if($arrJour[$numJour] . "_" . $depDuJour == $arrDepartsUser[$i]['r_epr_usr_depart']) {
                        ...
                        $strBlason = $arrDepartsUser[$i]['r_epr_usr_blason'];

                        if($strBlason == 'trispot') {
                            $strTrispotHTML = 'active';
                            $strTrispotChk = 'checked';
                        } else {
                            $strTrispotHTML = '';
                            $strTrispotChk = '';
                        }
            ...
            $strHTML .= '<div class="btn-group-sm btn-group-toggle custom-control-inline" data-toggle="buttons">';
            $strHTML .= '<label class="btn btn-warning '. $strTrispotHTML . '" id="trispotLbl">';
            $strHTML .= '<input type="checkbox" name="trispot' . $intNbDep . '" id="trispotChk" '. $strTrispotChk .' > TRISPOT';
            $strHTML .= '</label>';
            $strHTML .= '</div>';
