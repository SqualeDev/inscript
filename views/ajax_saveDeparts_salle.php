        ...
        if(!$objInscript->isConcoursClos($intConcours_ID)) {
            ...
            for ($i = 1; $i <= 10; $i++) {
                if(!empty($_POST['inputDepart' . $i])) {
                    if (isset($_POST['trispot' . $i]) && !empty($_POST['trispot' . $i])) {
                        $strBlason = "trispot";
                    } else {
                        $strBlason = "blason";
                    }
                    ...
                    $retour = $objInscript->insertNewInscription($strLicencie_ID, $intConcours_ID, $arrDat[1], $strBlason, $strTypeDepart, $arrDat[0]);
