...
<html>
    ...
    if (isset($_POST['username'])&&isset($_POST['mysql'])&&isset($_POST['db'])&&isset($_POST['username'])) {
        ...
        $sqlQuery = 'LOAD DATA LOCAL INFILE "'. $file . '"
            INTO TABLE ' . $table . '
            FIELDS TERMINATED by \',\'
            LINES TERMINATED BY \'\n\'
            IGNORE 1 ROWS
            (@epr_date_deb,@epr_date_fin,epr_orga,epr_lieu,epr_discipline,epr_caracteristique,epr_invit,epr_result,epr_compl)
            SET epr_date_deb = STR_TO_DATE(@epr_date_deb, \'%d/%m/%Y\'),
                epr_date_fin = STR_TO_DATE(@epr_date_fin, \'%d/%m/%Y\')
            ;';
        
        //mysqli_query( $cons, 'TRUNCATE TABLE "' . $table . '";'  ) or die ( mysql_error() );
        mysqli_query( $cons, $sqlQuery ) or die ( mysql_error() );
        ...
        echo "Success";
        echo "<b> total $count0 records have been added to the table $table </b> ";
    } else {
    ...
    2) If fields in your csv are not separated by commas go to Line 87 of php file and change the query<br>
    3) If each tuple in your csv are not one below other(i.e not seperated by a new line) go to line 88 of php file and change the query<br>
